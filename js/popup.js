(function(){
    ...
    // Loading effect CSS
    widget_css += ".gt-loading{position:relative;pointer-events:none}.gt-loading:after{content:'';position:absolute;top:50%;left:50%;width:12px;height:12px;margin:-6px 0 0 -6px;border:1px solid #ccc;border-top:1px solid #4285f4;border-radius:50%;animation:gt-spin .8s linear infinite}@keyframes gt-spin{0%{transform:rotate(0deg)}100%{transform:rotate(360deg)}}";
    ...
    if(url_structure == 'none') {
        ...
        window.doGTranslate = function(lang_pair){if(lang_pair.value)lang_pair=lang_pair.value;if(lang_pair=='')return;var lang=lang_pair.split('|')[1];if(get_current_lang() == null && lang == lang_pair.split('|')[0])return;var teCombo;var sel=document.getElementsByTagName('select');for(var i=0;i<sel.length;i++)if(sel[i].className.indexOf('goog-te-combo')!=-1){teCombo=sel[i];break;}if(document.getElementById('google_translate_element2')==null||document.getElementById('google_translate_element2').innerHTML.length==0||teCombo.length==0||teCombo.innerHTML.length==0){setTimeout(function(){doGTranslate(lang_pair)},500)}else{teCombo.value=lang;fire_event(teCombo,'change');fire_event(teCombo,'change');setTimeout(function(){document.querySelectorAll('.gt-loading').forEach(function(e){e.classList.remove('gt-loading')})},2000)}}
