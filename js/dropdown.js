...
(function(){
    ...
    console.log('GTranslate Dropdown: Checking cookie...', document.cookie);
    ...
    if(early_cookie_check && early_cookie_check[2] && early_cookie_check[2] !== '/auto/auto') {
        ...
        if(early_lang && early_lang !== early_default) {
            console.log('GTranslate Dropdown: Cookie detected with foreign language: ' + early_lang);
                ...
                setTimeout(function() {
                    ...
                    function checkPriorityTranslation() {
                        ...
                        if(elapsedTime < minWaitTime) {
                            console.log('GTranslate Dropdown: Waiting minimum time... ' + elapsedTime + 'ms');
                            setTimeout(checkPriorityTranslation, checkInterval);
                            return;
                        }
                        
                        // Enhanced translation completion check
                        var translationStatus = window.checkDropdownTranslationComplete ? window.checkDropdownTranslationComplete() : {isComplete: false, translatedElements: 0, progress: 0};
                        
                        console.log('GTranslate Dropdown: Translation check - ' + 
                        ...
                        if (translationStatus.isComplete) {
                            stableCheckCount++;
                            console.log('GTranslate Dropdown: Translation appears complete (' + stableCheckCount + '/' + requiredStableChecks + ')');
                            ...
                            if(priorityLoading) {
                                ...
                                console.log('GTranslate: Priority loading hidden after ' + elapsedTime + 'ms, elements: ' + translationStatus.translatedElements);
                            }
                        ...
                        if (elapsedTime >= maxWaitTime) {
                            console.log('GTranslate Dropdown: Translation timeout reached (' + maxWaitTime + 'ms) - hiding loading');
                            var priorityLoading = document.getElementById('gt-priority-loading');
                            if(priorityLoading) {
                                priorityLoading.remove();
                                console.log('GTranslate: Priority loading hidden after timeout ' + elapsedTime + 'ms');
                            }
            ...
            console.log('GTranslate Dropdown: Cookie detected but language is default: ' + early_lang);
            // Hide header-injected loading since no translation needed
            hideHeaderLoading();
        }
    } else {
        console.log('GTranslate Dropdown: No translation cookie found');
        // Hide header-injected loading since no translation needed
        hideHeaderLoading();
    }
    
    // Function to hide header-injected loading when no translation needed
    function hideHeaderLoading() {
        console.log('GTranslate Dropdown: hideHeaderLoading called');
        
        // Try multiple times to find header-injected elements
        var attempts = 0;
        var maxAttempts = 10;
        
        function tryHideElements() {
            attempts++;
            console.log('GTranslate Dropdown: Attempt ' + attempts + ' to find header elements');
            
            var instantLoading = document.getElementById('gt-instant-loading');
            var instantHide = document.getElementById('gt-instant-hide');
            
            console.log('GTranslate Dropdown: Found elements - loading:', !!instantLoading, 'hide:', !!instantHide);
            ...
            if (instantLoading) {
                ...
                console.log('GTranslate Dropdown: Removed header-injected loading (no translation needed)');
            }
            
            if (instantHide) {
                instantHide.remove();
                console.log('GTranslate Dropdown: Removed header-injected hide style');
            }
            ...
            if ((instantLoading || instantHide) || attempts >= maxAttempts) {
                console.log('GTranslate Dropdown: Finished hiding header elements after ' + attempts + ' attempts');
                return;
            }
            
            // Try again after a short delay
            setTimeout(tryHideElements, 10);
        }
    ...
    // Enhanced function to check translation completion for dropdown
    function checkDropdownTranslationComplete() {
    ...
    // Make function globally accessible for other parts of the script
...
(function(){
    ...
    // Spinner loading CSS (same as base.js)
    ...
    // Custom dropdown styling with globe icon on left - no border, better alignment
    widget_css += ".gt_selector{appearance:none!important;-webkit-appearance:none!important;-moz-appearance:none!important;background:white!important;border:none!important;outline:none!important;box-shadow:none!important;padding:12px 20px 12px 45px!important;font-size:16px!important;font-weight:500!important;color:#333!important;cursor:pointer!important;border-radius:8px!important;position:relative!important;min-width:160px!important;height:48px!important;line-height:24px!important;display:flex!important;align-items:center!important;background-image:url(\"data:image/svg+xml;charset=utf8,<svg width='20' height='20' viewBox='0 0 20 20' fill='none' xmlns='http://www.w3.org/2000/svg'><path d='M3 7.5H17M3 12.5H17M9.58333 2.5C8.17945 4.74968 7.43517 7.34822 7.43517 10C7.43517 12.6518 8.17945 15.2503 9.58333 17.5M10.4167 2.5C11.8205 4.74968 12.5648 7.34822 12.5648 10C12.5648 12.6518 11.8205 15.2503 10.4167 17.5M2.5 10C2.5 10.9849 2.69399 11.9602 3.0709 12.8701C3.44781 13.7801 4.00026 14.6069 4.6967 15.3033C5.39314 15.9997 6.21993 16.5522 7.12987 16.9291C8.03982 17.306 9.01509 17.5 10 17.5C10.9849 17.5 11.9602 17.306 12.8701 16.9291C13.7801 16.5522 14.6069 15.9997 15.3033 15.3033C15.9997 14.6069 16.5522 13.7801 16.9291 12.8701C17.306 11.9602 17.5 10.9849 17.5 10C17.5 8.01088 16.7098 6.10322 15.3033 4.6967C13.8968 3.29018 11.9891 2.5 10 2.5C8.01088 2.5 6.10322 3.29018 4.6967 4.6967C3.29018 6.10322 2.5 8.01088 2.5 10Z' stroke='%235D5D5D' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'/></svg>\")!important;background-repeat:no-repeat!important;background-position:left 15px center!important;background-size:20px 20px!important}.gt_selector:hover,.gt_selector:focus,.gt_selector:active{background-color:white!important;border:none!important;outline:none!important;box-shadow:none!important}.gt_selector option{width:277px!important;height:49px!important;padding:12px 16px!important;background-color:white!important;color:#333!important;font-size:16px!important;line-height:25px!important;display:flex!important;align-items:center!important;border:none!important;outline:none!important}.gt_selector option:hover,.gt_selector option:focus{background-color:#1976d2!important;color:white!important}.gt_selector option:active{background-color:#1976d2!important;color:white!important}";
    ...
    // No placeholder option - only language options
    ...
    document.querySelectorAll(u_class+' .gt_selector').forEach(function(e){e.addEventListener('change', function(evt){
        ...
        if(url_structure == 'none')
            ...
            // For sub-domain/sub-directory, redirect with small delay
            setTimeout(function() {
